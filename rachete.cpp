...
void Rachete::citesteDate(const std::string& fisier) {
    ...
    while (std::getline(in, linie)) {
        if (linie.empty() || linie.find("ID") != std::string::npos) continue;
        ...
        std::getline(ss, r.id, ';');
        std::getline(ss, token, ';'); r.capacitate_transport = std::stod(token);
        std::getline(ss, token, ';'); r.consum_combustibil = std::stod(token);
        std::getline(ss, token, ';'); r.pret_combustibil = std::stod(token);
